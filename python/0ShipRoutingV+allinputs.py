            ...
            for i in range(wind_u.shape[0]):
                for j in range(wind_v.shape[1]):
                    ...
                    if deno==0:
                        ...
                        if nume==0:
                            angle_rel[i,j]=0
                    angle_rel[i,j]=math.degrees(math.atan(nume/deno))       #A_WRef
                    if angle_rel[i,j]< 0:
...
def TotalResCosFun(Vs,L,Lwl,Los,B,TF,TA,CB,S,Dp,NRud,NBrac,NBoss,NThr,rho_sea,nu_sea,g):
   ...
   for j in range(a.size):                                                    #prod_help=[prod_help[0,i].^a[0,i]]
...
# it is a better choice to choose xrow=ycol
xrow=int(input('number of rows(>abs(sy-gy)):'))
ycol=int(input('number of columns(>abs(sx-gx):'))
## for wind resistance
...
# 设置四条边，
for i in range(0, xrow):        #north boundary
    ox.append(i)
    oy.append(xrow)
for i in range(0, ycol):         #east boundary
    ox.append(0)
    oy.append(i)
for i in range(0, xrow):        #south boundary
    ox.append(i)
    oy.append(0)
for i in range(0, ycol):         #west boundary
    ox.append(ycol)
    oy.append(i)

TotalCost=np.mat(np.zeros((1,abs(sx-gx)+abs(sy-gy))))
""" 
...
Vs_max=2*Vs_min
 """
show = False
for i in range(abs(sx-gx)+abs(sy-gy)):
    length=(abs(sx-gx)+abs(sy-gy))+i
    Vs=float(length/t)
    print('The current velocity is [m/s]:',Vs)
    ...
    rx, ry, TotalCost[0,i] = dijkstra.planning(sx, sy, gx, gy,CF_N,CF_E,CF_S,CF_W)
minx,miny= np.where(TotalCost == np.min(TotalCost))
length_E=(abs(sx-gx)+abs(sy-gy))+miny
Vs_E=float(length_E/t)
CFE_Wind_N,CFE_Wind_E,CFE_Wind_S,CFE_Wind_W = WindResCosFun(Vs_E,Axv,rho_air,wind_u,wind_v,Cair_extend)
CFE_m=TotalResCosFun(Vs_E,L,Lwl,Los,B,TF,TA,CB,S,Dp,NRud,NBrac,NBoss,NThr,rho_sea,nu_sea,g)
CFE_Wave_N,CFE_Wave_E,CFE_Wave_S,CFE_Wave_W=WaveResCosFun(Vs_E,Lbwl,B,rho_sea,wave_d,wave_h)
...
if show:
    ...
    print('The effective velocity is [m/s]:',Vs_E)
    print('The minimum cost is [W]:',TotalCost_min)
    plt.plot(rx, ry, '-r')
    plt.pause(0.01)
    plt.show()